...
export class HotelSystem {
    ...
    private reservations: Reserva[] = [];
  
    constructor(rooms: number) {
      this.rooms = rooms;
    }
 
    searchReservation(id: number): Reserva {
      ...
      getSortReservations(): Reserva[] {
        return [...this.reservations].sort((a, b) => {
          return new Date(a.checkIn).getTime() - new Date(b.checkIn).getTime();
        });
      }
    
      addReservation(reservation: Reserva): void {
      ...
      removeReservation(id: number): Reserva | undefined {
